...
import { getBlogPost, listBlogCategories, listBlogFiles, updateBlogPost } from '../../lib/repo'
import type { BlogCategory, BlogFile, BlogPost } from '../../lib/types'
...
export default function BlogPostClient() {
  ...
  if (!id) {
    return <div className="panel p-4">Missing post id.</div>
  }

  return editMode ? <BlogPostEdit id={id} /> : <BlogPostView id={id} />
}

function BlogPostView({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const files = await listBlogFiles(id, 'inline')
      if (cancelled) return
      setPost(res)
  ...
  const contentBlocks = useMemo(() => {
    ...
    const byName = new Map(inlineFiles.map((f) => [f.filename.toLowerCase(), f]))
    const blocks: Array<
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          {post && (
            <Link
              ...
              href={`/blog/post?id=${post.id}&edit=1`}
            >
              Edit
            </Link>
          )}
...
function BlogPostEdit({ id }: { id: string }) {
  const router = useRouter()
  const [active, setActive] = useState<string | null>(null)
  const [loading, setLoading] = useState(true)
  ...
  return (
    <AppShell left={<BlogSidebar active={active} onSelect={onSelectCategory} />}>
      <div className="panel p-4">
        <div className="flex items-center gap-2 text-sm">
          <Link className="border rounded px-2 py-1 hover:bg-white/10" href={`/blog/post?id=${id}`}>
            Back to Post
          </Link>
          <Link className="border rounded px-2 py-1 hover:bg-white/10" href="/blog">
            Back to Blog
